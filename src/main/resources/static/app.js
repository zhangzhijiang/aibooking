...
// API Configuration
const API_URL = document.getElementById('apiUrl').value || 'http://localhost:8080/api';
...
async function initializeSpeechSDK() {
    ...
    if (!speechKey || !speechRegion) {
        updateStatus('Please configure Azure Speech Service key and region', 'error');
        return false;
    }
    ...
    try {
        ...
        recognizer.recognizing = (s, e) => {
            if (e.result.text) {
                document.getElementById('transcription').textContent = e.result.text + '...';
            }
        };
        ...
        recognizer.recognized = async (s, e) => {
            if (e.result.reason === speechSdk.ResultReason.RecognizedSpeech) {
                ...
                // Send to backend API
                await processBookingRequest(text);
        ...
        updateStatus('Failed to initialize speech recognition. Please check your configuration.', 'error');
        return false;
    }
}
...
async function processBookingRequest(text) {
    try {
        const response = await fetch(`${API_URL}/bookMeeting`, {
            method: 'POST',
            headers: {
        ...
        updateStatus('Failed to process booking request: ' + error.message, 'error');
    }
}
...
function displayResponse(result) {
    ...
    if (result.status === 'success') {
        updateStatus('Meeting booked successfully!', 'success');
        
        let html = '<h3>✅ Booking Confirmation</h3>';
        ...
        if (result.startTime) {
            html += `<div class="response-item"><strong>Start Time:</strong> ${new Date(result.startTime).toLocaleString()}</div>`;
        }
        
        if (result.endTime) {
            html += `<div class="response-item"><strong>End Time:</strong> ${new Date(result.endTime).toLocaleString()}</div>`;
        }
        
        if (result.attendees && result.attendees.length > 0) {
            html += `<div class="response-item"><strong>Attendees:</strong> ${result.attendees.join(', ')}</div>`;
        }
        ...
        if (result.exceptions && result.exceptions.length > 0) {
            html += `<div class="response-item"><strong>Exceptions:</strong> ${result.exceptions.join(', ')}</div>`;
        }
